...
public:
    ...
    ) override
    {
        ...
        auto scene_probe = std::make_unique<squared::scene2d::Actor>();
        scene_probe->set_bounds(8.0F, 8.0F, 16.0F, 16.0F);
        squared::scene2d::Actor* scene_probe_pointer = scene_probe.get();
        [[maybe_unused]] squared::scene2d::Actor& attached_scene_probe =
            scene_.add_actor(std::move(scene_probe));
        scene2d_ready_ = scene_.hit(12.0F, 12.0F) == scene_probe_pointer;
        if (!scene2d_ready_) {
            SDL_Log("Scene2D hierarchy diagnostic failed");
        }
